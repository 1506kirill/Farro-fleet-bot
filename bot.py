...
1. Пользователь часто пишет только цифры машины, например 4553 или 8730.
1. car_id в JSON должен быть только из списка известных машин.
1. Если дата не указана - используй сегодняшнюю дату в формате DD.MM.YY.
1. ДАННЫЕ ДЛЯ ТАБЛИЦЫ ПИШИ НА РУССКОМ ЯЗЫКЕ.
1. Ответ должен быть ТОЛЬКО JSON.
1. Если не хватает важных данных - верни missing_fields.
1. Если пользователь пишет “ТО” или “плановое ТО”, description верни как “ТО”.
1. Если пользователь пишет про штраф, долг, должен, дожен - type верни как “liability_minus”.
1. Если пользователь пишет “взял”, “принял”, “погасил”, “дал” в контексте долга - type верни как “liability_plus”.
1. Для liability_minus и liability_plus odometer не нужен.
1. Для liability_minus и liability_plus description должна быть только текстом причины БЕЗ номера машины и БЕЗ суммы.
1. Для income description может быть пустым.
