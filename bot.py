...
def repair_all_vehicle_history() -> str:
    """
    One-time historical repair for every vehicle sheet and every year:
      - N remains income amount in USD;
      - O becomes mileage delta between current and previous positive payment;
      - missing expense odometers F are estimated by that vehicle's own history;
      - missing historical USD I/N values are filled by historical cash SELL rate;
      - every value written by this bot is colored blue.
    Existing manual F/I/N values are not overwritten.
    """
    spreadsheet = get_sheet()
    ...
    try:
        ...
        rate_error = e
        logger.error("Historical repair USD archive error: %s", e)
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        # Missing EXPENSE odometers and missing historical USD amounts.
        for row_idx, row in enumerate(rows[7:], start=8):
            expense_date = parse_short_date(row[4] if len(row) > 4 else None)
            expense_odo_raw = str(row[5]).strip() if len(row) > 5 else ""
            expense_desc = str(row[6]).strip() if len(row) > 6 else ""
            expense_uah = parse_money_float(row[7] if len(row) > 7 else None)
            expense_usd_raw = str(row[8]).strip() if len(row) > 8 else ""
        ...
        # Column O: correct historical mileage deltas for ALL years.
    ...
    result = (
        "✅ История автопарка исправлена.\n"
        f"🚗 Листов изменено: {changed_sheets}\n"
        f"🔵 Дорисовано одометров расходов F: {expense_odos}\n"
        f"🔵 Заполнено USD расходов I: {usd_expenses}\n"
        f"🔵 Заполнено USD приходов N: {usd_incomes}\n"
        f"🔵 Исправлено значений пробега O: {mileage_deltas}"
    )
    if rate_error:
        result += (
            "\n⚠️ Архив курса USD был недоступен, поэтому "
            "часть пустых I/N могла остаться без изменений."
        )
    return result
...
async def weekday_vehicle_completion_job(context: ContextTypes.DEFAULT_TYPE):
    """Monday-Thursday 18:30 Kyiv. Friday is handled by the combined Friday job."""
    try:
        result = await asyncio.to_thread(complete_manual_rows_all_vehicles)
...
async def friday_reports_update_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        completion_result = await asyncio.to_thread(
            complete_manual_rows_all_vehicles
        )
    ...
    try:
        if text.lower().strip() in {
            ...
            await update.message.reply_text(
                "⏳ Исправляю историю всех машин: колонку O, "
                "пустые одометры расходов и недостающие USD. "
                "Это может занять несколько минут."
            )
            result = await asyncio.to_thread(repair_all_vehicle_history)
