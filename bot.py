...
def rebuild_fleet_graphs(monthly_data=None) -> str:
    """
    Fill the existing 'Графики' sheet with compact source tables and create:
      1) fleet income/expense by month;
      2) current-month income/expense by vehicle;
      3) current-month expense share by vehicle;
      4) current-month profit by vehicle.
    """
    spreadsheet = get_sheet()
    ...
    # Last 12 months for fleet dynamics.
    months_all = list(iter_months_desc(
        today.year, today.month, oldest_date.year, oldest_date.month
    ))
    months_12 = list(reversed(months_all[:12]))

    fleet_rows = [["Месяц", "Приход $", "Расход $", "Чистые $"]]
    for y, m in months_12:
        income = 0.0
        expense = 0.0
        for car_id in KNOWN_CAR_IDS:
            vals = report_values_for_car(monthly_data, y, m, car_id)
            income += vals[0]
            expense += vals[1]
        fleet_rows.append([
            month_label(y, m),
    ...
    current_rows = [["Авто", "Приход $", "Расход $", "Чистые $"]]
    for car_id in KNOWN_CAR_IDS:
        income, expense, profit = report_values_for_car(
            monthly_data, today.year, today.month, car_id
        )
        current_rows.append([car_id, income, expense, profit])

    # Do not clear the whole sheet: only our data area and our embedded charts.
    graphs_ws.batch_clear(["A1:D40", "F1:I40"])
    graphs_ws.update(range_name="A1", values=fleet_rows, value_input_option="USER_ENTERED")
    graphs_ws.update(range_name="F1", values=current_rows, value_input_option="USER_ENTERED")

    _delete_existing_sheet_charts(spreadsheet, graphs_ws.id)

    # Google Sheets API chart ranges are zero-based, end-exclusive.
    fleet_end = len(fleet_rows)
    current_end = len(current_rows)
    sheet_id = graphs_ws.id

    def domain(col, end_row):
        return {
            "domain": {
                "sourceRange": {
                    "sources": [{
    ...
    chart_requests = [
                ...
                "chart": {
                    "spec": {
                        ...
                        "basicChart": {
                            ...
                            "domains": [domain(0, fleet_end)],
                            "series": [series(1, fleet_end), series(2, fleet_end)],
                            "headerCount": 1,
                        },
                    },
                    "position": {
                ...
                "chart": {
                    "spec": {
                        ...
                        "pieChart": {
                            ...
                            "domain": domain(5, current_end)["domain"],
                            "series": series(7, current_end)["series"],
        ...
        },
        {
            "addChart": {
                "chart": {
                    "spec": {
                        "title": f"Приходы и расходы по машинам — {month_label(today.year, today.month)}",
                        "basicChart": {
                            "chartType": "COLUMN",
                            "legendPosition": "BOTTOM_LEGEND",
                            "axis": [
                                {"position": "BOTTOM_AXIS", "title": "Авто"},
                                {"position": "LEFT_AXIS", "title": "USD"},
                            ],
                            "domains": [domain(5, current_end)],
                            "series": [series(6, current_end), series(7, current_end)],
                            "headerCount": 1,
                        },
                    },
                    "position": {
        ...
        },
        {
            "addChart": {
                "chart": {
                    "spec": {
                        "title": f"Чистая прибыль по машинам — {month_label(today.year, today.month)}",
                        "basicChart": {
                            "chartType": "BAR",
                            "legendPosition": "NO_LEGEND",
                            "axis": [
                                {"position": "BOTTOM_AXIS", "title": "USD"},
                                {"position": "LEFT_AXIS", "title": "Авто"},
                            ],
                            "domains": [domain(5, current_end)],
                            "series": [{
                                "series": {
...
def rebuild_reports_sheet_history() -> str:
    ...
    graphs_result = rebuild_fleet_graphs(monthly_data)
...
def update_current_month_report() -> str:
    ...
    return (
        ...
        f"Итог {current.year} пересчитан. {graphs_result}"
    )


async def friday_reports_update_job(context: ContextTypes.DEFAULT_TYPE):
    """Every Friday 18:30 Kyiv. No Telegram spam on success."""
    try:
        result = await asyncio.to_thread(update_current_month_report)
        logger.info("Friday report update: %s", result)
    except Exception as e:
        logger.error("Friday report update failed: %s", e, exc_info=True)
        for user_id in ALLOWED_USERS:
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"⚠️ Ошибка обновления листа отчетов: {e}",
                )
            except Exception:
                pass
