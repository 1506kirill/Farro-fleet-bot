...
def _longest_plausible_regulation_path(
    points: List[Tuple[date, int, str]],
) -> List[Tuple[date, int, str]]:
    """
    Longest chronologically plausible odometer path.

    Used separately for L and, only when necessary, for F.
    """
    if not points:
        return []

    # Deduplicate exact date/odometer pairs.
    dedup = {}
    for d, odo, source in points:
        dedup[(d, odo)] = source

    pts = [
        (d, odo, source)
        for (d, odo), source in dedup.items()
    ]
    ...
    def plausible(a, b) -> bool:
        ...
        # A very generous physical ceiling. It rejects impossible jumps while
        # allowing intensive fleet use.
        return (delta / days) <= 350
    ...
    # On ties prefer the path that ends later. Do NOT prefer the larger
    # odometer; that was one of the ways a bad high reading could win.
    end_i = max(
        range(n),
        key=lambda i: (
            best_len[i],
...
def _regulation_odometer_points(
    rows: List[List[str]],
) -> List[Tuple[date, int, str]]:
    """
    Trusted odometer timeline for oil/GRM.

    IMPORTANT SOURCE PRIORITY:
      L = regular driver/payment odometer -> primary truth source.
      F = expense/service odometer        -> secondary source.

    We first build a plausible path from L only.  A later F value can extend
    that path only when it is physically plausible from the latest trusted
    point.  Therefore a chain of erroneous/generated F values cannot replace a
    healthy L history and create false 300k+ overdue results.

    If a vehicle has no usable L history, we fall back to a cleaned F-only path.
    """
    l_points: List[Tuple[date, int, str]] = []
    f_points: List[Tuple[date, int, str]] = []

    for row in rows[7:]:
        d_f = parse_short_date(row[4] if len(row) > 4 else None)
        o_f = _regulation_odometer(row[5] if len(row) > 5 else None)
        if d_f and o_f is not None:
            f_points.append((d_f, o_f, "F"))

        d_l = parse_short_date(row[10] if len(row) > 10 else None)
        o_l = _regulation_odometer(row[11] if len(row) > 11 else None)
        if d_l and o_l is not None:
            l_points.append((d_l, o_l, "L"))

    trusted_l = _longest_plausible_regulation_path(l_points)

    if trusted_l:
        trusted = list(trusted_l)
        anchor_d, anchor_odo, _ = trusted[-1]

        # Only F readings AFTER the latest trusted L can make the current
        # odometer newer. Older F readings remain available to service search,
        # but they must not redefine the current odometer.
        later_f = sorted(
            [
                p for p in f_points
                if p[0] > anchor_d
        ...
        for d, odo, source in later_f:
            days = (d - last_d).days
            if days <= 0 or odo < last_odo:
                continue
            ...
            trusted.append((d, odo, source))
            last_d = d
            last_odo = odo

        return trusted

    # No regular payment odometers: use only a cleaned F path.
    return _longest_plausible_regulation_path(f_points)
...
def get_current_odometer_from_rows(rows: List[List[str]]) -> Optional[int]:
    ...
    Uses the latest plausible reading from cleaned F/L history instead of
    parse_num(), which could concatenate dates/text into gigantic odometers.
    """
    points = _regulation_odometer_points(rows)
    if not points:
        return None

    latest_date = max(p[0] for p in points)
    same_day = [p[1] for p in points if p[0] == latest_date]
    return max(same_day) if same_day else points[-1][1]
...
def find_last_service(rows: List[List[str]], mode: str) -> Tuple[Optional[str], Optional[int]]:
    """
    Find the latest credible oil/GRM service.

    Service history lives in F/G expense rows, while the current odometer is
    primarily anchored by L.  Therefore a valid service F reading must NOT be
    required to appear in the current-odometer path.

    A candidate is accepted when:
      - its description clearly matches oil/GRM;
      - date and odometer are valid;
      - service odometer is not above the trusted current odometer;
      - mileage growth from service to the latest trusted odometer is physically
        plausible (<= 350 km/day, with a small same-day tolerance).
    """
    ...
    current_date, current_odo, _ = points[-1]
    ...
    for block in blocks:
        ...
        service_date_raw = block[0]["date"]
        service_date = parse_short_date(service_date_raw)
        service_odo = _regulation_odometer(block[0]["odo"])

        if not service_date or service_odo is None:
            continue
        if service_date > current_date:
            continue
        if service_odo > current_odo:
            continue

        days = (current_date - service_date).days
        delta = current_odo - service_odo
        ...
        candidates.append((service_date, service_date_raw, service_odo))

    if not candidates:
        return None, None

    # Latest service by actual date, not merely by row position.
    candidates.sort(key=lambda x: x[0], reverse=True)
    _d, raw_date, odo = candidates[0]
    return raw_date, odo
...
def regulation_remaining_km(
    ...
    """
    Single source of truth for oil/GRM:
      returns (service_date, service_odo, current_odo, remaining_km).
    """
    interval = 10000 if mode == "oil" else 50000
    service_date, service_odo = find_last_service(rows, mode)
    current_odo = get_current_odometer_from_rows(rows)

    if service_odo is None or current_odo is None:
        return service_date, service_odo, current_odo, None

    current_odo = max(current_odo, service_odo)
    remaining = interval - (current_odo - service_odo)
    return service_date, service_odo, current_odo, remaining
