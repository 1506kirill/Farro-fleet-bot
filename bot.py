...
def rebuild_reports_sheet_history() -> str:
    ...
    Preserves the user's headers/formatting and writes only:
      - column A below the header: months
      - each recognized vehicle's 3 report columns
    """
    spreadsheet = get_sheet()
    ...
    end_row = data_start_row + len(months) - 1

    updates = []

    # Column A: month names, newest first.
    updates.append({
        "range": f"A{data_start_row}:A{end_row}",
        "values": [[month_label(y, m)] for y, m in months],
    })

    # Each vehicle: three columns Приход / Расход / Чистые.
    for car_id, start_col in car_columns.items():
        ...
        values = [
            report_values_for_car(monthly_data, y, m, car_id)
            for y, m in months
        ]
        updates.append({
...
def update_current_month_report() -> str:
    """
    Friday update: recalculate only the current month.
    If a new month has started and its row is absent, rebuild the report once
    so the new month appears at the top.
    """
    spreadsheet = get_sheet()
    reports_ws = get_reports_worksheet(spreadsheet)
    if not reports_ws:
    ...
    # Find current month in column A.
    col_a = reports_ws.col_values(1)
    target_row = None
    for row_idx in range(data_start_row, len(col_a) + 1):
        if str(col_a[row_idx - 1]).strip() == current_label:
            target_row = row_idx
            break

    # New calendar month: make sure it is added at the top in correct order.
    ...
    return f"✅ Текущий месяц '{current_label}' в отчете обновлен."
