...
1. 4-значне з переліку машин = НОМЕР АВТО.
1. 3800-4200 без одометра = оплата оренди (income).
1. “Взяв/принял X за CAR” = завжди income, X=сума, CAR=авто.
1. НЕ створюй liability_plus при “взяв/принял” + сума + машина.
1. Якщо одометр вже є в тексті — НЕ питай повторно.
1. Дата DD.MM.YY, якщо немає — сьогодні.
1. ДАНІ ДЛЯ ТАБЛИЦІ — РОСІЙСЬКОЮ.
1. “ТО” → description=“ТО”.
1. штраф/долг/должен → liability_minus.
...
lo = t.lower().strip()
# Точні команди
exact = {“масло”,“замена масла”,“заміна масла”,“масло?”,“масла”,
“то”,“плановое то”,“планове то”,“то?”,
“замена”,“техобслуживание”,“техобслуговування”}
if lo in exact: return True
# Часткові збіги (якщо написано просто “масло” як частину речення без авто)
oil_words = [“масло”,“замена масла”,“заміна масла”,“техобслуг”]
car_present = any(cid in t for cid in [“0349”,“0418”,“0665”,“0736”,“1457”,
“1875”,“2548”,“2993”,“3021”,“3531”,“4553”,“4715”,“4895”,
“5308”,“5725”,“6514”,“6843”,“7121”,“7935”,“8204”,“8391”,
“8730”,“9245”,“9489”,“9854”])
if not car_present and any(w in lo for w in oil_words):
return True
return False
...
“””
Обробляє голосові повідомлення через OpenAI Whisper.
Транскрибує аудіо і передає текст у звичайний handle_msg.
“””
uid = update.effective_user.id
if ALLOWED_USERS and uid not in ALLOWED_USERS:
...
try:
    # Завантажуємо голосовий файл з Telegram
    ...
    with open(tmp_path, "rb") as audio_file:
        transcript = openai_client.audio.transcriptions.create(
            model="whisper-1",
            file=audio_file,
            language="uk",        # Пробуємо українську
    ...
    if not text:
        await update.message.reply_text("❌ Не вдалося розпізнати мовлення. Спробуй ще раз.")
        return

    await update.message.reply_text(f"🎙 Розпізнано: «{text}»")

    # Підміняємо текст повідомлення і передаємо в основний обробник
