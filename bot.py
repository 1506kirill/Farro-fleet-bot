...
def lookup_insurer_office(company: str) -> Optional[Dict[str, Any]]:
    """
    Find the insurer's Dnipro office using OpenStreetMap/Nominatim.

    The result may include address, coordinates, phone and website when those
    fields are published in OpenStreetMap. Missing data is simply omitted.
    """
    ...
    queries = [
        f"{company}, Дніпро, Україна",
        f"{company}, Днепр, Украина",
        f"{company}, Україна",
    ]
    ...
    for query in queries:
        try:
            ...
            logger.warning("Insurance office lookup failed for %s: %s", query, e)
            continue

        if not items:
            continue

        # Prefer a Dnipro result when possible.
        chosen = None
        for item in items:
            display = str(item.get("display_name", "")).lower()
            if "дніпро" in display or "днепр" in display or "dnipro" in display:
                chosen = item
                break
    ...
    if send_details:
        ...
        if office:
            if office.get("address"):
                lines.append(f"📍 Адреса: {office['address']}")
            if office.get("phone"):
