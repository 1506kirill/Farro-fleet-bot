...
def _regulation_latest_point(
    ...
    """
    Return the latest factual odometer used for oil/GRM regulations.

    Source priority:
      1) L with date K = regular driver/payment odometer.
      2) F with date E = expense/service odometer, but ONLY if its date is
         later than the latest valid K/L reading.

    No path-building, interpolation or averaging is used here.
    The bot simply trusts the latest factual dated reading.
    """
    latest_l: Optional[Tuple[date, int, str]] = None
    latest_f: Optional[Tuple[date, int, str]] = None

    for row in rows[7:]:
        # Regular driver/payment reading: K date + L odometer.
        ...
        # Expense/service reading: E date + F odometer.
        ...
        if d_f and o_f is not None:
            point = (d_f, o_f, "F")
            if latest_f is None or (d_f, o_f) > (latest_f[0], latest_f[1]):
                latest_f = point
    ...
    # A service/expense odometer can replace L only when it is genuinely newer.
    if latest_f[0] > latest_l[0]:
        return latest_f

    # Same day or older F: prefer the regular payment odometer L.
    return latest_l
