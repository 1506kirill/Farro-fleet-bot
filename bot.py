...
def _regulation_latest_point(
    ...
    """
    Return the latest factual odometer for oil/GRM regulations.

    Priority:
      1) Latest valid K/L driver/payment odometer.
      2) A newer E/F service odometer may replace it ONLY if the increase from
         the latest K/L reading is physically plausible.

    This keeps 0736/5725 anchored to their latest real payments while still
    allowing cars such as 4715, which have no newer payments, to use a later
    service/GPS odometer from E/F.
    """
    latest_l: Optional[Tuple[date, int, str]] = None
    f_points: List[Tuple[date, int, str]] = []

    for row in rows[7:]:
        # K date + L odometer = regular driver/payment reading.
        ...
        if d_l and o_l is not None:
            point = (d_l, o_l, "L")
            if latest_l is None or (d_l, o_l) > (latest_l[0], latest_l[1]):
                latest_l = point

        # E date + F odometer = expense/service reading.
    ...
    if latest_l is None:
        # No driver/payment history: use the latest factual F reading.
        if not f_points:
            return None
        return max(f_points, key=lambda p: (p[0], p[1]))

    # Start from the latest trusted payment odometer.
    current = latest_l

    # Consider only F readings that are genuinely later than that payment.
    later_f = sorted(
        [p for p in f_points if p[0] > latest_l[0]],
        key=lambda p: (p[0], p[1]),
    )

    last_date, last_odo, _ = current

    for d_f, o_f, source in later_f:
        days = (d_f - last_date).days
        if days <= 0:
            continue

        delta = o_f - last_odo

        # Reject backwards readings and impossible jumps.
        # 350 km/day is intentionally generous for fleet use.
        if delta < 0:
            continue
        if (delta / days) > 350:
            continue

        current = (d_f, o_f, source)
        last_date, last_odo = d_f, o_f
...
def find_last_service(rows: List[List[str]], mode: str) -> Tuple[Optional[str], Optional[int]]:
    """
    Find the latest factual oil/GRM service by date from E/F/G.

    The current odometer is NOT used to choose the service row.
    We simply take the latest matching service entry with a valid date and
    odometer, which matches the way the fleet table is maintained manually.
    """
    ...
    # Latest actual service date wins.
...
def regulation_remaining_km(
    ...
    current_date = _regulation_current_date_from_rows(rows)

    if service_odo is None or current_odo is None:
        return service_date_raw, service_odo, current_odo, None

    # Do not let an accidentally future-dated service row distort the result.
    service_date = parse_short_date(service_date_raw)
    if service_date and current_date and service_date > current_date:
        return service_date_raw, service_odo, current_odo, None

    # If a service odometer is above the latest factual odometer, use the
    # service odometer as the current floor rather than creating negative usage.
