...
def _regulation_odometer_points(
    ...
    """
    Build ONE cleaned odometer timeline for oil/GRM calculations.

    Sources:
      F = expense/service odometer
      L = driver/payment odometer

    The previous implementation could still be misled by a pure-numeric bad
    odometer (for example a mistakenly generated 600k reading).  Here we find
    the longest chronologically plausible path through all strict numeric
    readings.  Isolated huge jumps therefore do not become the "current"
    odometer just because they are numeric.

    Plausibility:
      - odometer never goes backwards;
      - same-day difference <= 1,500 km;
      - dated growth <= 350 km/day.
    """
    raw_points: List[Tuple[date, int, str]] = []
    ...
    for row in rows[7:]:
        ...
        if d_l and o_l is not None:
            raw_points.append((d_l, o_l, "L"))

    if not raw_points:
        return []

    # Remove exact duplicates, prefer L over F for the same date/odometer.
    dedup: Dict[Tuple[date, int], str] = {}
    for d, odo, source in raw_points:
        key = (d, odo)
        if key not in dedup or source == "L":
            dedup[key] = source
    ...
    points.sort(key=lambda x: (x[0], x[1], 0 if x[2] == "L" else 1))
    ...
    # Longest plausible chronological path (DP).
    # Score heavily rewards number of consistent points, then L readings,
    # then recency.  A single isolated bad reading cannot beat a long real path.
    best_len = [1] * n
    best_l_count = [1 if p[2] == "L" else 0 for p in points]
    prev_idx = [-1] * n

    for i in range(n):
        for j in range(i):
            if not plausible(points[j], points[i]):
                continue
            cand_len = best_len[j] + 1
            cand_l = best_l_count[j] + (1 if points[i][2] == "L" else 0)
            if (
                cand_len > best_len[i]
                or (
                    cand_len == best_len[i]
                    and cand_l > best_l_count[i]
                )
            ):
                best_len[i] = cand_len
                best_l_count[i] = cand_l
                prev_idx[i] = j

    # Choose the strongest path; on ties prefer one ending later and with more L.
    end_i = max(
        range(n),
        key=lambda i: (
            best_len[i],
            best_l_count[i],
            points[i][0].toordinal(),
            points[i][1],
        ),
    )
    ...
    accepted = [points[i] for i in path_idx]

    logger.debug(
        "Regulation odometer path: accepted=%s of %s, last=%s",
        len(accepted),
        len(points),
...
def find_last_service(rows: List[List[str]], mode: str) -> Tuple[Optional[str], Optional[int]]:
    """
    Find last oil/GRM service only if its odometer belongs to the same cleaned
    regulation timeline used for the current odometer.

    This keeps manual commands and the 09:15 notification on one calculation
    path and prevents a bad F value from becoming a service anchor.
    """
    blocks = split_expense_blocks(rows)
    if not blocks:
        return None, None

    accepted_points = _regulation_odometer_points(rows)
    accepted_pairs = {(d, odo) for d, odo, _source in accepted_points}
    ...
    for block in reversed(blocks):
        ...
        service_odo = block[0]["odo"]

        if service_date and (service_date, service_odo) in accepted_pairs:
            return service_date_raw, service_odo
