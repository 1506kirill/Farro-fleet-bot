...
def backfill_historical_usd_buy_rates() -> str:
    """
    One-time mass update:
      E date -> I USD BUY rate
      K date -> O USD BUY rate
    Only I and O are changed.
    """
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        for row_idx, row in enumerate(rows[7:], start=8):
            # Expense block: E date -> I rate.
            expense_date = parse_short_date(row[4] if len(row) > 4 else None)
            expense_has_data = any(
                str(row[c]).strip() if len(row) > c else ""
                for c in range(4, 8)  # E:H
            )
            if expense_date and expense_has_data:
            ...
            # Income block: K date -> O rate.
            income_date = parse_short_date(row[10] if len(row) > 10 else None)
            income_has_data = any(
                str(row[c]).strip() if len(row) > c else ""
                for c in range(10, 14)  # K:N
            )
            if income_date and income_has_data:
    ...
    result = (
        ...
        f"💰 Колонка O (приходи): {total_o} значень"
    )
    if missing_dates:
...
def write_expense_rows(ws, date_value: str, odometer: int, items: List[Dict[str, Any]], usd_rate: Optional[float], odometer_estimated: bool):
    ...
    for item in items:
        ...
        usd_value = round(usd_rate, 2) if usd_rate else ""
    ...
    try:
        if text.lower().strip() in {
            ...
            await update.message.reply_text(
                ...
                "Зміню лише колонки I та O. Це може зайняти кілька хвилин."
            )
