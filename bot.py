...
def complete_manual_rows_all_vehicles() -> str:
    ...
    Every weekday at 18:30:
      - fill missing USD values using today's Dnipro cash USD SELL rate;
      - fill missing odometers with a per-vehicle date-based estimate;
      - color only cells written by the bot blue;
      - never overwrite manually filled odometers or USD values.
    """
    spreadsheet = get_sheet()
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        for row_idx, row in enumerate(rows[7:], start=8):
            # EXPENSE side: E date, F odometer, G description, H UAH, I USD.
            expense_date = parse_short_date(row[4] if len(row) > 4 else None)
            ...
            expense_has_operation = bool(
                expense_date and (
                    expense_desc or expense_uah is not None or expense_usd_raw
                )
            )
            ...
            if expense_has_operation:
                if (
                    not expense_usd_raw
                    and expense_uah is not None
                    and usd_rate is not None
                    and usd_rate > 0
                ):
                    usd_value = round(expense_uah / usd_rate, 2)
                    updates.append({
                        "range": f"I{row_idx}",
                        "values": [[usd_value]],
                    })
                    blue_cells.append(f"I{row_idx}")
                    usd_cells += 1
                ...
                if not expense_odo_raw:
                    ...
                    if estimated is not None:
                        ...
                        blue_cells.append(f"F{row_idx}")
                        odo_cells += 1
                    else:
                        skipped_odo += 1

            # INCOME side: K date, L odometer, M UAH, N USD.
            income_date = parse_short_date(row[10] if len(row) > 10 else None)
            income_odo_raw = str(row[11]).strip() if len(row) > 11 else ""
            income_uah = parse_money_float(row[12] if len(row) > 12 else None)
            income_usd_raw = str(row[13]).strip() if len(row) > 13 else ""

            income_has_operation = bool(
                income_date and (
                    income_uah is not None or income_usd_raw
                )
            )

            if income_has_operation:
                if (
                    not income_usd_raw
                    and income_uah is not None
                    and usd_rate is not None
                    and usd_rate > 0
                ):
                    usd_value = round(income_uah / usd_rate, 2)
                    updates.append({
                        "range": f"N{row_idx}",
                        "values": [[usd_value]],
                    })
                    blue_cells.append(f"N{row_idx}")
                    usd_cells += 1

                if not income_odo_raw:
                    estimated = estimate_missing_odometer_from_rows(
                        rows, income_date
                    )
                    if estimated is not None:
        ...
        if updates:
            ...
            for cell in blue_cells:
                apply_blue_text(ws, cell)
            sheets_changed += 1

    # New bot-written data can affect reports and service logic; invalidate cache.
    global REPORT_CACHE
    REPORT_CACHE = {"snapshot": None, "time": None}
