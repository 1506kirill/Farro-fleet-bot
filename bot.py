...
def _historical_vehicle_profitability_rows(
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        if (
            purchase_odometer is not None
            and latest_odometer is not None
            and latest_odometer >= purchase_odometer
        ):
        ...
        rows_out.append([
            full_plate,
            _format_report_money(purchase_price),
            purchase_date.strftime("%d.%m.%Y") if purchase_date else "",
            _format_report_km(driven_km),
            _format_report_money(initial_expenses),
            _format_report_money(running_expenses),
            _format_report_money(all_income),
            _format_report_money(historical_profit),
            downtime_days,
        ])
...
def rebuild_fleet_graphs(
    ...
    # Ensure hidden helper columns X:AG exist.
    required_cols = 33  # AG
    ...
    # Hide all technical helper columns X:AG.
