...
def _historical_vehicle_profitability_rows(
    ...
    """
    Build the visible historical USD profitability table.
    ...
    Source cells/columns per vehicle sheet:
      ...
      F/L = odometer history; latest = maximum value
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        vehicle_rows = _run_with_sheets_backoff(
            ws.get_all_values,
            operation_name=f"{car_id}: profitability read",
        )
        ...
        purchase_odometer = parse_num(cell_value(2, 2))       # B2
        ...
        for row in vehicle_rows[7:]:
            ...
            # N is the USD income column. O remains mileage delta.
            ...
            odo_expense = parse_num(
                row[5] if len(row) > 5 else None
            )
            odo_income = parse_num(
        ...
        driven_km: Any = ""
        if (
            purchase_odometer is not None and
            latest_odometer is not None and
            latest_odometer >= purchase_odometer
        ):
            driven_km = latest_odometer - purchase_odometer

        historical_profit: Any = ""
...
def rebuild_fleet_graphs(monthly_data=None) -> str:
    ...
    profitability_rows = _historical_vehicle_profitability_rows(
        spreadsheet,
        today,
    )

    # Ensure hidden helper columns X:AE exist.
    required_cols = 31  # AE
    ...
    # Hide all technical helper columns X:AE.
    ...
    profitability_end_index = profitability_end_row  # API end index is exclusive; 1-based row value works here.
    ...
    chart_requests = [
        ...
        # 2. Current-month expense share.
        {
            "addChart": {
                "chart": {
                    "spec": {
                        "title": (
                            f"Расходы по машинам — "
                            f"{month_label(today.year, today.month)}"
                        ),
                        "pieChart": {
        ...
        # 3. Historical net profit by vehicle, beside the visible table.
        {
            "addChart": {
                "chart": {
                    "spec": {
                            ...
                            "domains": [{
                                "domain": {
                                    "sourceRange": {
                                        "sources": [{
                                            "sheetId": sheet_id,
                                            "startRowIndex": profitability_start_row - 1,
                                            "endRowIndex": profitability_end_index,
...
def update_report_vehicle_headers_and_borders(
    ...
    """
    Keep only the report vehicle headers and vertical vehicle separators in sync.

    Safety boundaries:
      * does not clear or rewrite report data;
      * changes only cells that already contain a known vehicle number in the
        report header area;
      * changes only the RIGHT border of each 3-column vehicle block.
    """
    if not car_columns:
        return

    # The subheader is immediately above data_start_row. Vehicle numbers may be
    # on that row or up to three rows above it (same rule as detect_report_layout).
    subheader_row = data_start_row - 1
    first_header_row = max(1, subheader_row - 3)
    header_values = reports_ws.get(
        f"A{first_header_row}:ZZ{subheader_row}"
    )

    header_updates = []
    for rel_row, row in enumerate(header_values):
        sheet_row = first_header_row + rel_row
        for col_idx, value in enumerate(row, start=1):
            car_id = extract_digits(str(value))
            if car_id not in car_columns:
                continue
            # Only update the header cell belonging to this detected car block.
            if col_idx != car_columns[car_id]:
                continue
            full_plate = VEHICLE_MAP.get(car_id, car_id)
            if str(value).strip() != full_plate:
    ...
    # A medium right border after every 3-column vehicle block visually
    # separates one car from the next without touching values or other borders.
    safe_end_row = max(int(end_row or 0), subheader_row)
    for _car_id, start_col in sorted(car_columns.items(), key=lambda x: x[1]):
        separator_col = start_col + 2
        separator_letter = gspread.utils.rowcol_to_a1(
            1, separator_col
        ).rstrip("1")
        format_cell_range(
            reports_ws,
            f"{separator_letter}{first_header_row}:{separator_letter}{safe_end_row}",
            CellFormat(
                borders={
