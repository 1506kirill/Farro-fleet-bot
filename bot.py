...
MINFIN_URL = "https://minfin.com.ua/currency/dnepropetrovsk/"
...
def get_usd_black_rate_dnipro() -> Optional[float]:
    """Return Minfin Dnipro USD cash-market BUY rate (Наличный курс -> USD -> Покупка)."""
    headers = {
    ...
    # Parse only the table that follows the cash-rate heading. This avoids
    # accidentally taking the bank-average BUY rate from the table above it.
    ...
    if cash_heading:
        ...
        if table:
            for tr in table.find_all("tr"):
                ...
                if cells and cells[0].strip().upper() == "USD" and len(cells) >= 2:
                    m = re.search(r"([0-9]{2}[.,][0-9]{1,4})", cells[1])
                    if m:
                        rate = float(m.group(1).replace(",", "."))
                        if 20 <= rate <= 100:
                            return rate

    # Safe fallback for flattened markup: require the cash-rate section and USD
    # row explicitly; never fall back to an arbitrary number on the page.
    text = soup.get_text(" ", strip=True)
    m = re.search(
        r"(?:Наличный курс|Готівковий курс).*?USD\s+([0-9]{2}[.,][0-9]{1,4})",
        text,
...
def _parse_minfin_archive_buy_rates(html: str) -> Dict[date, float]:
    """Parse Minfin Dnipro auction archive table: Date | Spread | Buy | Sell | NBU."""
    ...
    for tr in soup.find_all("tr"):
        ...
        m = re.search(r"(\d{2}(?:\.\d{1,4})?)", buy_raw)
        if not m:
            continue
        try:
    ...
    if not rates:
        ...
        for ds, buy_raw in pattern.findall(flat):
            d = parse_short_date(ds)
            if not d or buy_raw == "-.--":
                continue
            try:
                rate = float(buy_raw.replace(",", "."))
            except ValueError:
                continue
            if 20 <= rate <= 100:
                rates[d] = rate
...
def load_minfin_dnipro_buy_archive() -> Dict[date, float]:
    """
    Load Minfin black-market USD BUY archive for Dnipro.
    ...
    for url in urls:
        try:
            ...
            resp.raise_for_status()
            parsed = _parse_minfin_archive_buy_rates(resp.text)
            if parsed:
    ...
    if not merged:
        raise RuntimeError(
            "Не вдалося отримати архів курсу купівлі USD Minfin для Дніпра. "
            + ("; ".join(errors[:2]) if errors else "")
        )
    return merged


def historical_buy_rate_for_date(target: date, rates: Dict[date, float]) -> Optional[float]:
    """
    Exact date first. If Minfin has no BUY quote that day (weekend/empty row),
...
def backfill_historical_usd_buy_rates() -> str:
    ...
    """
    rates = load_minfin_dnipro_buy_archive()
    spreadsheet = get_sheet()
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        for row_idx, row in enumerate(rows[7:], start=8):
            ...
            if expense_date and expense_amount is not None:
                rate = historical_buy_rate_for_date(expense_date, rates)
            ...
            if income_date and income_amount is not None:
                rate = historical_buy_rate_for_date(income_date, rates)
                if rate is not None and rate > 0:
    ...
    result = (
        "✅ Історичні курси купівлі USD оновлено.\n\n"
...
def write_single_action_to_sheet(data: dict, raw_text: str = "") -> str:
    ...
    try:
        ...
        if usd_rate:
            usd_note = f"\n💱 Курс для расчёта: {usd_rate}; USD-сумма записана в I/N"
    except Exception as e:
    ...
    try:
        if text.lower().strip() in {
            ...
            await update.message.reply_text(
                "⏳ Оновлюю історичний курс купівлі USD у Дніпрі. "
