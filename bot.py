...
def build_oil_report() -> str:
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        lines.append(f"{icon} {car_id} | {last_date} | {last_odo} | {format_km(remaining)} км")
    return "\n".join(lines)
...
async def check_service_and_insurance_notifications(context: ContextTypes.DEFAULT_TYPE):
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        if oil_odo is not None and current_odo is not None:
            ...
            if remaining <= 1000:
                messages.append(f"🚗 {car_id} — масло через {format_km(remaining)} км")
        ...
        if car_id not in SKIP_GRM:
            ...
            if grm_odo is not None and current_odo is not None:
                ...
                if remaining <= 1000:
                    messages.append(f"🚗 {car_id} — ГРМ через {format_km(remaining)} км")
        ...
        if best:
            ...
            days_left = (end_date - today).days
            if days_left <= 14:
                messages.append(f"🚗 {car_id} — страховка через {days_left} дн. ({company})")
