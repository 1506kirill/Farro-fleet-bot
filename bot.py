...
def _regulation_latest_point(
    ...
    """
    Determine the latest known factual odometer for oil/GRM regulations.

    Expense block:
      E = date
      F = odometer

    Income/payment block:
      K = date
      L = odometer

    Rule:
      1) Find the latest valid E/F point.
      2) Find the latest valid K/L point.
      3) Compare their dates.
      4) The point with the newer date is the current odometer.
      5) If the dates are equal, use the larger odometer.

    We do NOT take the maximum odometer across the whole history.
    """
    latest_expense: Optional[Tuple[date, int, str]] = None
    latest_income: Optional[Tuple[date, int, str]] = None

    for row in rows[7:]:
        # Expense/service side: E date + F odometer.
        expense_date = parse_short_date(
            row[4] if len(row) > 4 else None
        )
        expense_odo = _regulation_odometer(
            row[5] if len(row) > 5 else None
        )
        if expense_date and expense_odo is not None:
            point = (expense_date, expense_odo, "F")
            if (
                latest_expense is None
                or expense_date > latest_expense[0]
                or (
                    expense_date == latest_expense[0]
                    and expense_odo > latest_expense[1]
                )
            ):
                latest_expense = point

        # Income/payment side: K date + L odometer.
        income_date = parse_short_date(
            row[10] if len(row) > 10 else None
        )
        income_odo = _regulation_odometer(
            row[11] if len(row) > 11 else None
        )
        if income_date and income_odo is not None:
            point = (income_date, income_odo, "L")
            if (
                latest_income is None
                or income_date > latest_income[0]
                or (
                    income_date == latest_income[0]
                    and income_odo > latest_income[1]
                )
            ):
                latest_income = point
    ...
    # Same date: larger odometer wins.
    return (
        latest_expense
        if latest_expense[1] >= latest_income[1]
        else latest_income
    )
...
def get_vehicle_rows_from_snapshot(
    snapshot: Dict[str, List[List[str]]],
    car_id: str,
) -> Optional[List[List[str]]]:
    """Return vehicle sheet rows from an already loaded snapshot."""
    full_plate = VEHICLE_MAP.get(car_id, "")
    for title, rows in snapshot.items():
        if car_id in str(title) or (full_plate and full_plate in str(title)):
            return rows
    return None


def get_insurance_record_from_rows(
    rows: List[List[str]],
) -> Optional[Tuple[date, str]]:
    """
    Return the newest known insurance expiry date and company.

    Sources remain exactly the same as before:
      - A4
      - insurance history R/S
    """
...
def _drive_credentials():
    """Create Drive-authorized service-account credentials from existing GOOGLE_CREDS."""
    creds_dict = json.loads(GOOGLE_CREDS)
    scopes = ["https://www.googleapis.com/auth/drive.readonly"]
    return Credentials.from_service_account_info(creds_dict, scopes=scopes)


def find_insurance_file_in_drive(car_id: str) -> Optional[Dict[str, str]]:
    """
    Search files visible to the same Google service account.

    A file is returned only when:
      - its name contains this car number / plate digits, AND
      - the name looks like an insurance/policy document.

    This prevents accidentally sending an unrelated vehicle document.
    """
    full_plate = VEHICLE_MAP.get(car_id, "")
    plate_digits = extract_digits(full_plate)
    insurance_words = (
        "страх", "полис", "поліс", "осаго", "osago",
        "автоцив", "insurance", "цивіл",
    )
    ...
    search_terms = []
    for term in [car_id, plate_digits]:
    ...
    for term in search_terms:
        ...
        candidates.extend(resp.json().get("files", []))
    ...
    for item in unique.values():
        ...
        low = name.lower()
        digits = extract_digits(name)

        has_car = car_id in digits or (plate_digits and plate_digits in digits)
        looks_insurance = any(word in low for word in insurance_words)

        if not (has_car and looks_insurance):
            continue
...
def download_drive_file(file_info: Dict[str, str]) -> Tuple[io.BytesIO, str]:
    """Download a regular Drive file so Telegram can send it as an attachment."""
    file_id = file_info["id"]
    name = str(file_info.get("name") or "insurance.pdf")
    mime_type = str(file_info.get("mimeType") or "")

    # Insurance documents are expected to be normal PDF/image files.
    # Google-native Docs are deliberately not guessed/exported.
    if mime_type.startswith("application/vnd.google-apps."):
        raise RuntimeError(
            "Файл страховки является Google-документом, а не PDF/изображением"
        )
...
def lookup_insurer_office(company: str) -> Optional[Dict[str, Any]]:
    """
    Find ONLY the insurer's office in Dnipro using OpenStreetMap/Nominatim.

    Important:
      - Never fall back to Kyiv or another Ukrainian city.
      - If no Dnipro office is found, return None.
      - Phone/website are included only when published for the Dnipro result.
    """
    company = str(company or "").strip()
    if not company or company.lower() == "страховка":
        return None

    headers = {
        "User-Agent": "FarroAutoparkBot/1.0 (insurance office lookup)",
        "Accept-Language": "uk,ru;q=0.9,en;q=0.7",
    }

    # Search Dnipro only. No Ukraine-wide fallback.
    queries = [
        f"{company}, Дніпро, Дніпропетровська область, Україна",
        f"{company}, Днепр, Днепропетровская область, Украина",
    ]

    def is_dnipro_result(item: Dict[str, Any]) -> bool:
        ...
        combined = f"{display} {city_fields}"
        ...
        # Explicitly reject Kyiv or other clearly wrong large-city matches.
        wrong_city = any(
            token in combined
            for token in (
                "київ",
        ...
        return has_dnipro and not wrong_city
    ...
    for query in queries:
        try:
            ...
            logger.warning(
                "Insurance Dnipro office lookup failed for %s: %s",
                query,
                e,
            )
            continue

        # STRICT: choose only a Dnipro result.
        dnipro_items = [item for item in items if is_dnipro_result(item)]
        if not dnipro_items:
            continue

        chosen = dnipro_items[0]

        extras = chosen.get("extratags") or {}
        phone = (
            extras.get("contact:phone")
            or extras.get("phone")
            or extras.get("contact:mobile")
        )
        website = (
            extras.get("contact:website")
            or extras.get("website")
        )
        ...
        return {
            "address": str(chosen.get("display_name", "")).strip(),
    ...
    # Never substitute Kyiv/Ukraine-wide office.
    return None


async def send_insurance_package(
    update: Update,
    car_id: str,
    *,
    ...
    """
    Send insurance information for one vehicle.

    Used by:
      - command "страховка 3021"
      - the one-number vehicle card "3021"
    """
    snapshot = get_data_snapshot()
    rows = get_vehicle_rows_from_snapshot(snapshot, car_id)

    if not rows:
        if send_details:
            await update.message.reply_text(
                f"❌ Машину {full_plate_from_short(car_id)} не знайдено в таблиці"
            )
        return

    record = get_insurance_record_from_rows(rows)
    if not record:
        if send_details:
            await update.message.reply_text(
                f"🛡 {full_plate_from_short(car_id)}: даних про страховку немає."
            )
        return

    end_date, company = record
    today = datetime.now(KYIV_TZ).date()
    days_left = (end_date - today).days

    if send_details:
        if days_left < 0:
            validity = f"прострочена на {abs(days_left)} дн."
        elif days_left == 0:
            validity = "закінчується сьогодні"
        else:
            validity = f"діє ще {days_left} дн."

        lines = [
            f"🛡 Страховка {full_plate_from_short(car_id)}",
            f"📅 Діє до: {end_date.strftime('%d.%m.%Y')} — {validity}",
            f"🏢 Страхова: {company}",
        ]

        office = await asyncio.to_thread(lookup_insurer_office, company)
        if office:
            if office.get("address"):
                lines.append(f"📍 Адреса у Дніпрі: {office['address']}")
            if office.get("phone"):
                lines.append(f"☎️ Телефон: {office['phone']}")
            if office.get("website"):
                lines.append(f"🌐 Сайт: {office['website']}")
        else:
            lines.append("📍 Офіс страхової у Дніпрі не знайдено.")

        await update.message.reply_text("\n".join(lines))

        if (
            office
            and office.get("lat") is not None
            and office.get("lon") is not None
        ):
            try:
                ...
                logger.warning("Insurance location send failed: %s", e)

    if send_file:
        try:
            file_info = await asyncio.to_thread(find_insurance_file_in_drive, car_id)
            if file_info:
                bio, filename = await asyncio.to_thread(
                    download_drive_file,
                    file_info,
                )
                try:
                ...
                await update.message.reply_text(
                    "📄 Файл страховки в Google Drive не найден. "
                    "Бот ищет файл по номеру машины и словам "
                    "«страховка/полис/ОСАГО/автоцивілка» в названии."
                )
        except Exception as e:
            logger.error(
                "Insurance file send failed for %s: %s",
                car_id,
                e,
                exc_info=True,
            )
            if send_details:
                await update.message.reply_text(
                    f"⚠️ Не вдалося надіслати файл страховки: {e}"
                )
...
async def _run_notify(context: ContextTypes.DEFAULT_TYPE):
    ...
    if alert_items:
        ...
        text = "✅ Стан регламентів на сьогодні: все в порядку."
        for user_id in ALLOWED_USERS:
            try:
                await context.bot.send_message(chat_id=user_id, text=text)
                logger.info("No-alert notification sent to %s", user_id)
            except Exception as e:
                logger.error("No-alert notification send error: %s", e)
...
def detect_insurance_car_request(text: str) -> Optional[str]:
    """Recognize 'страховка 3021', 'страховка по 3021', full plate, etc."""
    t = str(text or "").strip()
    low = t.lower()
    if not (
        low.startswith("страховка")
        or low.startswith("страхування")
        or low.startswith("страховка?")
    ):
        return None
    ...
    for car_id in KNOWN_CAR_IDS:
        ...
        if re.search(rf"(?<!\d){re.escape(car_id)}(?!\d)", t):
            return car_id
        if full_plate and full_plate.lower() in low:
            return car_id
    return None
...
def is_regulations_report_request(text: str) -> bool:
    t = str(text or "").lower().strip()
    return t in {"регламенты", "регламенти", "регламент"}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_regulations_report_request(text):
            # Full manual report: show ALL known regulations, not only urgent ones.
            oil_report = build_oil_report()
            grm_report = build_grm_report()
            insurance_report = build_insurance_report()
            await update.message.reply_text("🛢 Стан масла:\n\n" + (oil_report or "Немає даних"))
            await update.message.reply_text("⚙️ Стан ГРМ:\n\n" + (grm_report or "Немає даних"))
            await update.message.reply_text("🛡 Страховка:\n\n" + (insurance_report or "Немає даних"))
            return
        ...
        insurance_car_id = detect_insurance_car_request(text)
        if insurance_car_id:
            await send_insurance_package(
                update,
                insurance_car_id,
                send_details=True,
                send_file=True,
        ...
        if re.match(r"^\d{4}$", text_stripped) and text_stripped in KNOWN_CAR_IDS:
            ...
            # Keep the existing vehicle card unchanged and add the insurance
            # attachment after it. Detailed insurer contacts are available via
            # the explicit command "страховка 3021".
            await send_insurance_package(
                update,
                car_id,
                send_details=False,
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• грм\n"
        f"• страховка\n"
        f"• регламенты (масло + ГРМ + страховка по всем машинам)\n"
