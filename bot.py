...
def _regulation_odometer_points(
    ...
    """
    Build a cleaned F/L odometer timeline for regulation calculations.

    F = expense/service odometer
    L = driver payment odometer

    Obviously impossible jumps are rejected. L points are considered highly
    trustworthy because they are regular driver payment readings, but F points
    remain usable when a vehicle has no recent payments (e.g. a long idle car).
    """
    ...
    # Same date: keep all initially; sorting L first gives payment readings
    # preference as a local reference point.
    raw_points.sort(key=lambda x: (x[0], 0 if x[2] == "L" else 1, x[1]))

    # First, create trusted payment anchors. Reject impossible payment jumps.
    l_points = [(d, o, s) for d, o, s in raw_points if s == "L"]
    trusted_l: List[Tuple[date, int, str]] = []
    for point in l_points:
        if not trusted_l:
            trusted_l.append(point)
            continue

        prev_d, prev_o, _ = trusted_l[-1]
        d, odo, _ = point
        days = (d - prev_d).days
        if days <= 0:
            if d == prev_d and odo >= prev_o:
                trusted_l[-1] = point
            continue

        delta = odo - prev_o
        # A real odometer should not go backwards and a fleet car cannot
        # realistically add more than 350 km/day over a dated interval.
        if delta < 0 or delta / days > 350:
            continue
        trusted_l.append(point)

    # Validate F service readings against the nearest trusted L anchors where
    # possible. This prevents a single bad/generated F value from producing
    # hundreds of thousands of false overdue kilometres.
    accepted: List[Tuple[date, int, str]] = list(trusted_l)
    ...
    for d, odo, source in raw_points:
        ...
        before = None
        after = None
        for ld, lo, _ in trusted_l:
            if ld <= d:
                before = (ld, lo)
            if ld >= d and after is None:
                after = (ld, lo)

        plausible = True

        if before:
            days = max((d - before[0]).days, 0)
            if odo < before[1]:
                plausible = False
            elif days == 0:
                if abs(odo - before[1]) > 1500:
                    plausible = False
            elif (odo - before[1]) / days > 350:
                plausible = False

        if plausible and after:
            days = max((after[0] - d).days, 0)
            if odo > after[1]:
                if days == 0:
                    if abs(odo - after[1]) > 1500:
                        plausible = False
                elif (odo - after[1]) / days > 350:
                    plausible = False

        if plausible:
            accepted.append((d, odo, "F"))

    # If there are no payment anchors at all, fall back to a conservative
    # sequential F-only cleanup so idle cars still work.
    if not trusted_l:
        accepted = []
        f_points = sorted(
            [(d, o, s) for d, o, s in raw_points if s == "F"],
        ...
        for point in f_points:
            if not accepted:
                ...
                continue
            prev_d, prev_o, _ = accepted[-1]
            d, odo, _ = point
            days = (d - prev_d).days
            if days < 0:
                continue
            if days == 0:
                if odo >= prev_o and odo - prev_o <= 1500:
                    accepted[-1] = point
                continue
            delta = odo - prev_o
            if delta < 0 or delta / days > 350:
