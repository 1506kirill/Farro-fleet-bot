...
async def friday_reports_update_job(context: ContextTypes.DEFAULT_TYPE):
    """Every Friday 18:30 Kyiv: update the report and the two requested charts."""
    try:
        ...
        logger.info("Friday report update: %s | %s", report_result, graphs_result)
    except Exception as e:
        logger.error("Friday report/graphs update failed: %s", e, exc_info=True)
        for user_id in ALLOWED_USERS:
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"⚠️ Ошибка обновления отчета/графиков: {e}",
                )
            except Exception:
                pass
